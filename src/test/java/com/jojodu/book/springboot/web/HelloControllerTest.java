...
@WebMvcTest // 여러 스프링 테스트 어노테이션 중, Web(Spring MVC)에 집중할 수 있는 어노테이션
            ...
            // 여기서 컨트롤러만 사용하기 떄문에 사용한다.
public class HelloControllerTest {
    @Autowired // 스프링이 관리하는 Bean을 주입 받는다.
    public MockMvc mvc; // 웹 API를 테스트할 때 사용한다. 스프링 MVC테스트의 시작점.
...
//                                                        // 응답 본문의 내용을 검증한다.
    ...
    public void helloDto가_리턴된다() throws Exception{
        ...
        mvc.perform(get("/hello/dto")
                            .param("name", name) // API테스트할 때 사용될 요청 파라미터를 설정한다. 단, 값은 String만 허용됨.
                            .param("amount", String.valueOf(amount)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", is(name))) // JSON응답값을 필드별로 검증할 수 있는 메소드이다.
                                                                  // $를 기준으로 필드명을 명시한다.
